...
export const LinkAnchor = styled(NavLink)`
  ...
  &.active {
    ...
    svg {
      fill: ${(props) =>
		props.color === "light" ? props.theme.colors?.lightTheme.mainColor : props.theme.colors?.darkTheme.mainColor};
    }
    p {
      color: ${(props) =>
		props.color === "light" ? props.theme.colors?.darkTheme.mainColor : props.theme.colors?.lightTheme.mainColor};
    }
  }
`;
